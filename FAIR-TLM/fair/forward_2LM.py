...
def fair_scm(
    ...
    kerosene_supply=0.,
    landuse_forcing='co2',
    year_PD = 2011,   # year used for present day aerosol forcing
    af_PD = -0.9,   # aerosol foring in
    scale_aerosol = False
    ):
    ...
    if useMultigas:
        ...
        # scale the aerosol foring by Present day aerosol forcing
        if scale_aerosol == True:
            from .RCPs import rcp85
            years = rcp85.Emissions.year
            F_PD = F[years==year_PD,8]
            af1765 = F[years ==1765, 8]
            scale_aer = af_PD/(F_PD-af1765)
            F[:,8] = (F[:,8]-af1765)*scale_aer
    ...
    if restart_in == False:
        ...
        T_j[0,0] = T[0] + ( np.sum(F[0,:]) - lambda_*T[0] - epsilon * gamma * (T[0] - 0)) / C_u
        T_j[0,1] = 0 + gamma * (T[0] - 0) / C_d
    ...
    for t in range(1,nt):
        ...
        if useMultigas:
            ...
            # 3. Temperature
            # Update the thermal response boxes
            T_j[t,0] = T_j[t-1,0] + ( np.sum(F[t-1,:]) - lambda_*T_j[t-1,0] - epsilon * gamma * (T_j[t-1,0] - T_j[t-1,1])) / C_u
            T_j[t,1] = T_j[t-1,1] + gamma * (T_j[t-1,0] - T_j[t-1,1]) / C_d
