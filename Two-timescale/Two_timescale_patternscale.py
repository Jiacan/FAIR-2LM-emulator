...
zos_sm = xr.concat((zos_anom_sm.sel(time=slice(year_start, year_end),scenarios=b'rcp26'),zos_anom_sm.sel(time=slice(year_start, year_end),scenarios=b'rcp45'),zos_anom_sm.sel(time=slice(year_start, year_end),scenarios=b'rcp85')), dim='time')
for n in range(nlat):
    for m in range(nlon):
